...
Two ready-to-use workflows that run the TinyEdge engine on **real edge devices** from your pipeline. Both `uses:` the published `PhysicalSystems/tinyedge-actions` actions and need only a `TINYEDGE_API_KEY` repo secret + a device online during the run.
